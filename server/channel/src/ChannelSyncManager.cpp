...
ScriptEngine& ScriptEngine::Using<ChannelSyncManager>() {
  if (!BindingExists("ChannelSyncManager", true)) {
    ...
    Sqrat::DerivedClass<ChannelSyncManager, DataSyncManager,
                        Sqrat::NoConstructor<ChannelSyncManager>>
        binding(mVM, "ChannelSyncManager");
    binding.Func("GetWorldEventCounter",
                 &ChannelSyncManager::GetWorldEventCounter);
...
bool ChannelSyncManager::Initialize() {
  ...
  cfg->UpdateHandler =
      &DataSyncManager::Update<ChannelSyncManager, objects::SearchEntry>;
  ...
  cfg->SyncCompleteHandler =
      &DataSyncManager::SyncComplete<ChannelSyncManager,
  ...
  cfg->UpdateHandler =
      &DataSyncManager::Update<ChannelSyncManager, objects::EventCounter>;
  ...
  cfg->SyncCompleteHandler =
      &DataSyncManager::SyncComplete<ChannelSyncManager, objects::MatchEntry>;
  ...
  cfg->UpdateHandler =
      &DataSyncManager::Update<ChannelSyncManager, objects::PentalphaMatch>;
  ...
  const std::set<std::string> worldTypes = {
      ...
      "InstanceAccess", "Match",          "MatchEntry",        "PentalphaEntry",
      "PentalphaMatch", "PvPMatch",       "SearchEntry",       "StatusEffect",
      "UBResult",       "UBTournament"};
...
std::list<std::shared_ptr<objects::SearchEntry>>
ChannelSyncManager::GetSearchEntries(objects::SearchEntry::Type_t type) {
...
std::shared_ptr<objects::EventCounter> ChannelSyncManager::GetWorldEventCounter(
    int32_t type) {
  std::lock_guard<std::mutex> lock(mLock);
...
namespace channel {
template <>
int8_t ChannelSyncManager::Update<objects::SearchEntry>(
  ...
  if (!success) {
    if (isRemove) {
      ...
      entryList.sort([](const std::shared_ptr<objects::SearchEntry>& a,
  ...
  if (success) {
    ...
    if (isApp) {
      auto parentType =
          (objects::SearchEntry::Type_t)((int8_t)entry->GetType() - 1);
    ...
    for (int32_t cid : cids) {
      ...
      if (client) {
        ...
        auto packetCode =
            ChannelToClientPacketCode_t::PACKET_SEARCH_ENTRY_REGISTER;
        switch (entry->GetLastAction()) {
          case objects::SearchEntry::LastAction_t::UPDATE:
            // Only clan search entries are ever actually updated, all others
            // are re-registered
            if (entry->GetType() != objects::SearchEntry::Type_t::CLAN_JOIN &&
                entry->GetType() !=
                    objects::SearchEntry::Type_t::CLAN_RECRUIT) {
              packetCode =
                  ChannelToClientPacketCode_t::PACKET_SEARCH_ENTRY_UPDATE;
            }
            break;
    ...
    if (isApp &&
        entry->GetLastAction() == objects::SearchEntry::LastAction_t::ADD &&
        parent) {
      auto client = parent ? server->GetManagerConnection()->GetEntityClient(
                                 parent->GetSourceCID(), true)
                           : nullptr;
      if (client) {
        libcomp::Packet notify;
        notify.WritePacketCode(
            ChannelToClientPacketCode_t::PACKET_SEARCH_APPLICATION);
...
void ChannelSyncManager::SyncComplete<objects::CharacterLogin>(
  ...
  for (auto& objPair : objs) {
    auto record =
        std::dynamic_pointer_cast<objects::CharacterLogin>(objPair.first);
    if (objPair.second) {
  ...
  mServer.lock()->GetAccountManager()->UpdateLogins(updates, removes);
}
    ...
    if (account) {
      ...
      if (client) {
        ...
        if (eCounter->GetPreExpireType()) {
          ...
          if (state->GetEventCounters(eCounter->GetPreExpireType()).Get() ==
              eCounter) {
            state->SetEventCounters(eCounter->GetPreExpireType(), NULLUUID);
          }
        } else {
          // Update state
...
void ChannelSyncManager::SyncComplete<objects::InstanceAccess>(
  ...
  for (auto& objPair : objs) {
    auto record =
        std::dynamic_pointer_cast<objects::InstanceAccess>(objPair.first);
    ...
    for (auto request : requested) {
      if (!zoneManager->CreateInstance(request)) {
        LogDataSyncManagerError([&]() {
          return libcomp::String(
                     "Failed to create zone instance from access request for "
                     "instnace type: %1\n")
...
void ChannelSyncManager::SyncComplete<objects::MatchEntry>(
  ...
  for (auto& objPair : objs) {
    auto entry = std::dynamic_pointer_cast<objects::MatchEntry>(objPair.first);
    if (objPair.second) {
...
int8_t ChannelSyncManager::Update<objects::StatusEffect>(
  ...
  if (isRemove) {
    LogDataSyncManagerError([&]() {
      return libcomp::String("Attempted to sync a status effect removal: %1\n")
    ...
    auto character = std::dynamic_pointer_cast<objects::Character>(
        libcomp::PersistentObject::GetObjectByUUID(effect->GetEntity()));
    auto account = character ? std::dynamic_pointer_cast<objects::Account>(
                                   libcomp::PersistentObject::GetObjectByUUID(
                                       character->GetAccount()))
                             : nullptr;
    if (account) {
      ...
      if (client) {
        ...
        StatusEffectChange eff(effect->GetEffect(), (int8_t)effect->GetStack(),
                               true);
        ...
        server->GetCharacterManager()->AddStatusEffectImmediate(client, cState,
                                                                changes);
